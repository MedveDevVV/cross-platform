...
public class AutoServiceFacade {
    ...
    public int countAvailablePlacesOnDate(LocalDate date) {
        ...
        try{
            ...
            throw new RuntimeException("Прервано ожидание потоков", e);
        }
    ...
    public Optional<CarServiceMaster> getMasterByOrderId(UUID orderId) {
        Optional<RepairOrder> order = repairOrderService.getOrderById(orderId);
        return order.map(RepairOrder::getCarServiceMaster);
    }
}
